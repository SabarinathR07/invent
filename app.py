...
class BlogPost(db.Model):
    ...
    qual = db.Column(db.Integer, nullable=False, default=0)
...
@app.route('/mpost', methods=['GET', 'POST'])
def mpost():
    ...
    if request.method == 'POST':
        ...
        new_post = BlogPost(title=post_title, author=post_author, content=post_content, ptype=post_ptype, pcor=post_pcor, pric=post_pric)
        db.session.add(new_post)
        db.session.commit()
        return redirect('/mpost')
    else:
        all_posts = BlogPost.query.order_by(BlogPost.date_posted).all()
        return render_template('mpost.html', posts=all_posts)
...
def postsz():
    ...
    if request.method == 'POST':
        ...
        new_post = BlogPost2(mid=post_mid,title=post_title, address=post_address, timetamp=ptime)
        db.session.add(new_post)
        db.session.commit()
        return redirect('/postsz')
    else:
        all_posts = [BlogPost2.query.order_by(BlogPost2.timetamp).all(),BlogPost.query.order_by(BlogPost.date_posted).all()]
...
def new_postz():
    if request.method == 'POST':
        ...
        new_postz = BlogPost2(mid=post_mid,title=post_title, address=post_address)
